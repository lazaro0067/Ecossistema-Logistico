...
# 3. Funções Auxiliares e de Leitura
...
def render_estoque_dia(operacao):
    st.subheader("📦 Gestão de Estoque & DOI Atual")
    df = carregar_estoque_consolidado(operacao)
    if df is not None and not df.empty:
        st.dataframe(df, use_container_width=True)
        render_botoes_download(df, f"Estoque_Consolidado_{operacao}")
    else:
        st.info("ℹ️ Nenhum dado de estoque carregado. Utilize a aba de Cadastros & Atualização de Bases.")

# 4. Módulo Completo de Gestão de Ressuprimento com Todas as Abas
def render_gestao_ressuprimento(operacao):
    ...
    with tab_m1:
        st.markdown("### 📁 Cadastros & Atualização de Bases (Estoque do Dia & Puxadas)")
    ...
    with tab_m2:
        render_estoque_dia(operacao)

    with tab_m3:
        st.markdown("### 🛒 Sugestão de Compra & Marcação por Dia")
        st.info("Módulo de sugestão de compra e marcação por dia integrado.")

    with tab_m4:
        st.markdown(f"### 📊 Gestão Ressuprimento (Cestas & Metas) - {nome_exibicao_op}")
