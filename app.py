...
# -----------------------------------------------------------------------------
# GESTÃO DE RESSUPRIMENTO & ACOMPANHAMENTO DE PUXADA
# -----------------------------------------------------------------------------
def render_gestao_ressuprimento(operacao):
    ...
    with tab_m1:
        ...
        df_metas = pd.read_sql_query(
            f"SELECT * FROM metas_ressuprimento_mensal WHERE operacao IN ({placeholders}) AND ano={ano_sel}",
            conn,
            params=nombres_filtro,
        )
        conn.close()
        ...
        if not df_diario.empty and meses_selecionados:
            ...
            for m_s in meses_selecionados:
                if m_s in [1, 3, 5, 7, 8, 10, 12]:
                    dias_no_mes_total += 31
                elif m_s in [4, 6, 9, 11]:
                    dias_no_mes_total += 30
                else:
                    dias_no_mes_total += (
                        29
                        if (
            ...
            df_diario_mes["volume_sellin_hl_num"] = df_diario_mes[
                "volume_sellin_hl"
            ].apply(parse_br_float)
            df_res_mes = (
                df_diario_mes.groupby("cesta")["volume_sellin_hl_num"]
                .sum()
                .reset_index()
            )
            ...
            df_metas_sel = df_metas[df_metas["mes"].isin(meses_selecionados)]
            df_metas_grp = (
                df_metas_sel.groupby("cesta")["meta_volume_hl"]
                .sum()
                .reset_index()
            )
            ...
            fator_tend = (
                (dias_no_mes_total / dias_preenchidos)
            ...
            df_comp["HL REAL MÊS"] = df_comp["volume_sellin_hl_num"]
            ...
            st.markdown(
                ...
                <div style="background-color: #0d2149; color: white; padding: 12px 20px; border-radius: 8px 8px 0 0; margin-bottom: 0px;">
                    ...
                    <span style="font-size: 13px; color: #b0c4de;">Somatório de HL · Meses: {meses_str} · {dias_preenchidos} dia(s) computado(s)</span>
                </div>
                """,
    ...
    with tab_m3:
        ...
        st.caption(
            "Selecione o arquivo e confirme abaixo as colunas corretas do seu relatório (evitando puxar valores financeiros)."
        )
        ...
        if f_ress_daily is not None:
            try:
                df_up = robust_read_file(f_ress_daily)
                st.success(
                    f"Arquivo carregado! ({len(df_up)} linhas encontradas)"
                )
                cols_disponiveis = list(df_up.columns)

                c_up1, c_up2 = st.columns(2)
                with c_up1:
                    col_op_sel = st.selectbox(
                        "Coluna de Operação:",
                        cols_disponiveis,
                        index=0 if len(cols_disponiveis) > 0 else 0,
                    )
                    col_hl_sel = st.selectbox(
                        "Coluna de Volume (HL / Sell-in):",
                        cols_disponiveis,
                        index=min(2, len(cols_disponiveis) - 1),
                    )
                with c_up2:
                    col_cesta_sel = st.selectbox(
                        "Coluna de Cesta / Indicador:",
                        cols_disponiveis,
                        index=min(4, len(cols_disponiveis) - 1),
                    )
                    col_data_sel = st.selectbox(
                        "Coluna de Data:",
                        cols_disponiveis,
                        index=min(5, len(cols_disponiveis) - 1),
                    )

                if st.button("🚀 Processar e Atualizar Base de Dados"):
                    conn = sqlite3.connect("puxada_ambev.db")
                    cursor = conn.cursor()
                    dt_now = datetime.now().strftime("%d/%m/%Y %H:%M")
                    registros_salvos = 0
                    ...
                    for _, r in df_up.dropna(
                        subset=[col_cesta_sel, col_data_sel]
                    ).iterrows():
                        raw_op = (
                            str(r[col_op_sel]).strip()
                            if pd.notna(r[col_op_sel])
                            else operacao
                        )
                        ...
                        cst_val = str(r[col_cesta_sel]).strip()
                        dt_val = str(
                            pd.to_datetime(
                                r[col_data_sel], errors="coerce"
                            ).strftime("%Y-%m-%d")
                        )
                        if dt_val == "NaT":
                            continue
                        ...
                        s_hl = parse_br_float(r[col_hl_sel])
        ...
        with sub_ress[0]:
            ...
            with c1:
                f_01 = st.file_uploader(
                    "Upload 01.11", type=["xlsx", "xls", "csv"], key="up_01"
                )
            ...
            with c2:
                f_lin = st.file_uploader(
                    "Upload Linear", type=["xlsx", "xls", "csv"], key="up_lin"
                )
            ...
            with c3:
                f_02 = st.file_uploader(
                    "Upload 02.03.04", type=["xlsx", "xls", "csv"], key="up_02"
                )
