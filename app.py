...
# =========================================================
# LÓGICA DE PROCESSAMENTO DAS COLUNAS A, C, E, F & CONSOLIDAÇÃO BAHIA
# =========================================================
def processar_base_ressuprimento(uploaded_file, ano=2026, mes=8):
    ...
    # Coluna A = Operação | C = HL | E = Indicador | F = Data
    ...
    return df_mapped[(df_mapped['data'].dt.month == mes) & (df_mapped['data'].dt.year == ano)].copy()
...
def gerar_relatorio_puxada(df_filtrado, ano=2026, mes=8):
    ...
    df_agrupado = df_filtrado.groupby(['operacao', 'indicador'])['hl_puxado'].sum().reset_index()

    def calcular_tabela_op(df_op, nome_op):
        indicadores = ['Cerveja', 'Nab', 'Match', 'Cerveja RGB', 'Nab Zero', 'Cerveja Zero Alcool', 'High End']
        linhas = []
        ...
        for ind in indicadores:
            ...
            media_diaria = real / dias_preenchidos if dias_preenchidos > 0 else 0.0
        ...
        tot_t = (tot_r / dias_preenchidos) * dias_totais_mes if dias_preenchidos > 0 else 0.0
    ...
    df_barreiras = df_agrupado[df_agrupado['operacao'].str.contains('Barreiras', case=False, na=False)]
    df_samavi = df_agrupado[df_agrupado['operacao'].str.contains('São Félix|Samavi', case=False, na=False)]
    ...
    tabela_diaria = df_filtrado.pivot_table(
        index='data',
        columns='operacao',
        values='hl_puxado',
        aggfunc='sum',
    ...
    return tabela_barreiras, tabela_samavi, tabela_bahia, tabela_diaria, dias_preenchidos
