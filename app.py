...
def render_gestao_ressuprimento(operacao):
    ...
    # TAB 1: ACOMPANHAMENTO MENSAL, ANUAL E SOMA CERVEJA + NAB
    with tab_m1:
        ...
        df_diario = pd.read_sql_query(
            f"SELECT * FROM gestao_ressuprimento_diario WHERE operacao='{operacao}' AND strftime('%Y', data_registro)='{ano_sel}'",
            conn
        )
        df_metas = pd.read_sql_query(
            f"SELECT * FROM metas_ressuprimento_mensal WHERE operacao='{operacao}' AND ano={ano_sel}",
            conn
        )
        conn.close()
        ...
        if not df_diario.empty:
            ...
            df_res_mes = df_diario_mes.groupby("cesta").agg({
                "volume_sellin_hl": "sum",
                "volume_real_hl": "sum"
            }).reset_index()

            df_metas_mes = df_metas[df_metas["mes"] == mes_sel]
            df_comp_mes = pd.merge(
                pd.DataFrame({"cesta": cestas_oficiais}),
                df_res_mes,
                on="cesta",
                how="left"
            ).fillna(0)
            ...
            df_comp_mes = pd.merge(
                df_comp_mes,
                df_metas_mes[["cesta", "meta_volume_hl"]],
                on="cesta",
                how="left"
            ).fillna(0)
            ...
            vol_nab_meta = df_comp_mes[df_comp_mes["cesta"] == "CATEGORIA_AGRUPADO - NAB"]["meta_volume_hl"].sum()
            vol_total_meta = vol_cerveja_meta + vol_nab_meta

            st.markdown(f"#### 🏆 Resumo Consolidado ({mes_ano_str})")
            k1, k2, k3 = st.columns(3)
            k1.metric("🍺 Cerveja Real / Meta", f"{vol_cerveja_real:,.1f} HL", f"Meta: {vol_cerveja_meta:,.1f} HL")
            k2.metric("🥤 NAB Real / Meta", f"{vol_nab_real:,.1f} HL", f"Meta: {vol_nab_meta:,.1f} HL")
            
            pct_ating = (vol_total_real / vol_total_meta * 100) if vol_total_meta > 0 else 0
            k3.metric("📦 VOLUME TOTAL (Cerveja + NAB)", f"{vol_total_real:,.1f} HL", f"Atingimento: {pct_ating:.1f}%")
            ...
            df_display = pd.concat([df_comp_mes, row_total], ignore_index=True)
            ...
            st.dataframe(df_piv_anual.style.format("{:,.1f}"), use_container_width=True)

        else:
            st.info("ℹ️ Nenhum dado diário encontrado para este ano. Faça o upload do relatório na aba 'Upload & Atualização'.")

    # TAB 2: CONFIGURAÇÃO DE METAS MENSAIS POR CESTA
    with tab_m2:
        st.markdown("### 🎯 Cadastrar / Ajustar Metas Mensais por Cesta")
        ...
        df_exist_metas = pd.read_sql_query(
            f"SELECT cesta, meta_volume_hl FROM metas_ressuprimento_mensal WHERE operacao='{operacao}' AND ano={ano_meta} AND mes={mes_meta}",
            conn
        )
        conn.close()
        ...
        with st.form("form_cad_metas"):
            st.markdown(f"**Metas em HL para {mes_ano_meta_str}:**")
            input_metas = {}
            for cst in cestas_oficiais:
                val_init = float(dict_metas_exist.get(cst, 0.0))
                input_metas[cst] = st.number_input(f"Meta: {cst}", min_value=0.0, value=val_init, step=10.0)
